...
- Ein Spieler hat im Laufe der Saison seine Trikotnummer gewechselt: Trainingsbewertungen zeigen die aktuelle Trikotnummer (bestehende Regel aus 001), Veo-Stats zeigen die zum jeweiligen Spielzeitpunkt historisch gültige Trikotnummer (bestehende Regel aus 004) — beide Tabs können denselben realen Spieler daher unter unterschiedlichen Trikotnummern führen; dies ist bestehendes, akzeptiertes Verhalten der jeweiligen Ursprungsfeatures und wird durch dieses Feature nicht aufgelöst.
...
- **SC-004**: Die im Veo-Stats-Tab gezeigten Summen entsprechen bei stichprobenartiger Nachrechnung exakt der Summe aller synchronisierten, einer Trikotnummer zugeordneten Veo-Werte seit `season_start` desselben Teams (0 Abweichungen) — unabhängig davon, ob die login-geschützte Saison-Übersicht aus 004 zum jeweiligen Zeitpunkt bereits denselben Saison-Zuschnitt anwendet.
