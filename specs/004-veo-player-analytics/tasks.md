...
- [ ] T010 [US1] Extend `app/server/api/veo/sync.post.ts`: fetch each team's active roster once before its match loop (research.md §5); per match, call T008 *before* opening the transaction (so a fetch failure skips the whole match — match + team stats included, FR-015/research.md §13, same as an incomplete Veo analysis), map via T009, and upsert into `veo_player_match_stats` with `ON CONFLICT (match_id, veo_jersey_number, stat_type) DO UPDATE` — `player_id` only overwritten when the existing row's `matched_manually` is `false` (research.md §10) — inside the same per-match transaction as the existing match/team-stats upsert (depends on T001-T004, T008, T009)
- [ ] T011 [P] [US1] Extend `app/composables/useVeoAnalytics.ts`: `listMatches()`'s nested select gains `veo_player_match_stats(veo_jersey_number, player_id, matched_manually, stat_type, category, value, players(id, name, jersey_number))`, filtered to `player_id is not null` for display; add `computePlayerSeasonSummary(matches)`, summing `value` by `player_id`/`stat_type` across all matches (research.md §6, same pattern as `computeSeasonSummary`)
