...
Ergänzt nach manuellem Test gegen den echten Veo-Account des Vereins (siehe research.md §§1-2 für die dabei korrigierten API-Annahmen).

- Q: Auf der Analytics-Seite werden alle synchronisierten Spiele untereinander gestapelt angezeigt — bei einer vollen Saison unübersichtlich. Wie stattdessen? → A: Immer nur ein Spiel gleichzeitig anzeigen, ausgewählt über eine Dropdown-Auswahl aller synchronisierten Spiele (Standard: das zuletzt gespielte).
- Q: Dasselbe gilt für die Spieler-Saison-Übersicht auf dem Dashboard — wie soll die Auswahl dort funktionieren? → A: Standardmäßig ebenfalls nur ein Spieler gleichzeitig, per Dropdown ausgewählt. Zusätzlich MUSS es möglich sein, bis zu vier Spieler gleichzeitig zum Vergleich auszuwählen; die jeweils beste (höchste) von den ausgewählten Spielern erreichte Kennzahl wird pro Zeile hervorgehoben.
- Q: Soll der manuelle Vergleich die einzige Möglichkeit sein, herausragende Spieler zu erkennen? → A: Nein — zusätzlich und unabhängig von der manuellen Auswahl MUSS pro Kennzahl automatisch eine Bestenliste der Top 3 Spieler angezeigt werden; bei Gleichstand auf dem dritten Platz werden alle gleichauf liegenden Spieler mit angezeigt (die Liste kann dadurch länger als 3 sein).
- Q: Rohwerte wie "2700" (Spielzeit in Sekunden) oder "27,8" (Geschwindigkeit) sind ohne Einheit uneindeutig — wie anzeigen? → A: Mit passender Einheit: Distanz in Metern ("m"), Geschwindigkeiten in km/h, gespielte Zeit von Sekunden in ganze Minuten umgerechnet ("min"). Reine Zähl-Kennzahlen (Sprints, Tore, Torschüsse, …) bleiben ohne Einheit.
- Q: Gilt die Einzelauswahl/Vergleichs-Logik (siehe oben) auch für die Einzelspiel-Ansicht auf der Analytics-Seite, oder nur für die Saison-Übersicht auf dem Dashboard? → A: Beide — dieselbe Dropdown-Auswahl + Bis-4-Vergleich + automatische Bestenliste gilt identisch für die Spieler-Aufschlüsselung eines einzelnen Spiels wie für die Saison-Übersicht.
- Q: Soll die automatische Bestenliste (FR-019) nur Kader-Spieler mit bestehender Zuordnung enthalten, oder auch (noch) nicht zugeordnete Trikotnummern? → A: Ein Trainer soll die Bestenliste immer sehen, auch wenn zu einer Trikotnummer noch kein Kader-Spieler zugeordnet ist — dargestellt als "#<Nummer> Nicht zugeordnet", nie mit einem geratenen Namen (SC-004). Für reguläre Mitglieder bleibt es bei FR-002/FR-011: ohne Zuordnung keine Anzeige.
- Q: Ein Kader-Spieler wurde erst angelegt, nachdem seine Spiele schon synchronisiert wurden — soll die Zuordnung stattdessen per Live-Join gegen die aktuelle Kaderliste aufgelöst werden (statt der beim Sync fest gespeicherten Zuordnung, FR-003)? → A: Nein — ein Live-Join würde genau das Problem reproduzieren, das FR-003 bewusst verhindert (Trikotnummern werden über eine Saison hinweg wiederverwendet; ein September-Spiel dürfte nicht rückwirkend dem Spieler zugeordnet werden, der heute die Nummer trägt). Stattdessen: eine Trikotnummer-Zuordnung kann in einem Schritt auf alle noch unzugeordneten Spiele desselben Teams angewendet werden (FR-022) — schreibt weiterhin echte, überschreibbare Zeilen, bleibt aber die Handarbeit von "einmal statt pro Spiel".
- Q: Sollen Spielernamen in den Veo-Statistik-Anzeigen anklickbar sein? → A: Ja — jeder angezeigte, zugeordnete Spielername verlinkt auf sein Spielerprofil (FR-023), analog zur bestehenden Verlinkung in der Ranking-Tabelle. Nicht zugeordnete Trikotnummern bleiben unverlinkt.
...
- **FR-017**: System MUST auf der Analytics-Seite genau ein Spiel gleichzeitig mit seiner Spieler-Aufschlüsselung anzeigen, ausgewählt über eine Dropdown-Auswahl aller synchronisierten Spiele des Teams (Standardauswahl: das zuletzt gespielte Spiel) — nicht alle Spiele gleichzeitig untereinander.
- **FR-018**: System MUST sowohl auf dem Dashboard (Saison-Übersicht) als auch auf der Analytics-Seite (Einzelspiel-Aufschlüsselung, FR-017) standardmäßig genau einen Spieler gleichzeitig anzeigen, ausgewählt über eine Dropdown-Auswahl. System MUST es zusätzlich an beiden Stellen ermöglichen, bis zu vier Spieler/Trikotnummern gleichzeitig auszuwählen und ihre Kennzahlen nebeneinander zu vergleichen; für jede Kennzahl MUST die unter den aktuell verglichenen Einträgen höchste erreichte Ausprägung optisch hervorgehoben werden.
- **FR-019**: System MUST unabhängig von der manuellen Auswahl (FR-018) für jede kuratierte Kennzahl automatisch eine Bestenliste der Top 3 anzeigen (Saison-Summe auf dem Dashboard, Einzelspiel-Wert auf der Analytics-Seite). Liegen mehrere Einträge auf dem dritten Platz gleichauf, MUST die Liste um alle gleichauf liegenden Einträge erweitert werden, statt sie abzuschneiden. Für einen Trainer MUST die Bestenliste zusätzlich Trikotnummern ohne Kader-Zuordnung einschließen (angezeigt als "#<Nummer> Nicht zugeordnet", nie mit einem geratenen Namen, SC-004) — für reguläre Mitglieder gilt weiterhin FR-002/FR-011: keine Anzeige ohne Zuordnung. Platz 1 MUST golden, Platz 2 silbern und Platz 3 bronzefarben hervorgehoben werden.
- **FR-020**: System MUST Kennzahlen mit ihrer Einheit anzeigen, nicht als bloße Zahl: Distanz in Metern, Höchst- und Durchschnittsgeschwindigkeit in km/h, gespielte Zeit umgerechnet von Sekunden in ganze Minuten. Reine Zähl-Kennzahlen (Sprints, hochintensive Läufe, Torschüsse, Tore, Torbeteiligungen) bleiben unitless.
- **FR-021** (US3): System MUST auch die Trikotnummer-Zuordnung (User Story 3) auf genau eine Trikotnummer gleichzeitig beschränken, ausgewählt über eine Dropdown-Auswahl — nicht alle offenen Zuordnungen eines Spiels gleichzeitig untereinander auflisten.
- **FR-022** (US3): System MUST es einem Trainer ermöglichen, eine Trikotnummer-Zuordnung in einem Schritt auf alle noch nicht zugeordneten Spiele desselben Teams anzuwenden (nicht nur das gerade angezeigte Spiel) — für den Fall, dass ein Kader-Spieler erst angelegt wird, nachdem dessen Spiele bereits synchronisiert wurden. Bereits (automatisch oder manuell) anders zugeordnete Zeilen MUST davon unberührt bleiben, da dieselbe Trikotnummer innerhalb derselben Saison zu unterschiedlichen Zeitpunkten legitim verschiedenen Spielern gehören kann.
- **FR-023** (US1, US2): System MUST jeden angezeigten, einem Kader-Spieler zugeordneten Namen (Saison-Übersicht, Bestenliste, Einzelspiel-Vergleich) als Link zum Spielerprofil dieses Spielers darstellen. Nicht zugeordnete Trikotnummern ("Nicht zugeordnet") MUST nicht verlinkt werden.
