...
- [x] T030 [P] Add RLS negative tests: `rls-negative-single-team.spec.ts` N11 (player write denied, V4) and N13 (no `select` policy on `veo_sync_credentials` for anyone, V1, against a real existing row); `rls-negative-cross-team.spec.ts` X11/X12 (trainer of team A cannot write team B's mapping/credentials, V2/V3)
- [x] T031 [P] Add RLS positive test: trainer of team A can `insert` then `update` (upsert) both tables for team A; can `select` `veo_team_mappings` (not `veo_sync_credentials`) for team A — added inline in `rls-negative-single-team.spec.ts` (N11-N13), doubling as the positive control for N11/N13's denial checks
...
- [x] T034 Hand-write the RLS migration `supabase/migrations/20260922200000_veo_trainer_self_service.sql`: drop the deny-all policies on `veo_team_mappings`/`veo_sync_credentials`; add the `is_trainer(team_id)`-gated `select`/`insert`/`update` policies per [contracts/rls-policies.md](./contracts/rls-policies.md)
...
- [x] T036 [P] Implement `app/server/utils/veo/login.ts` — `captureSessionViaLogin(email, password)`: headless Chromium, fill+submit the real `app.veo.co` login form, extract `auth.veo.co` cookies on success, throw a specific error on a detected failure state, hard timeout, `browser.close()` in `finally` (research.md §9). **Not live-tested against the real Veo login form** (avoided touching the sandbox's live production session during research) — needs one real smoke test before relying on it in production, per quickstart.md.
...
- [x] T038 Implement `app/server/api/veo/login.post.ts`: require a session, `requireTrainer(useAdminDb(), team_id, userId)` (no table write to attach RLS to at this step, see contracts/rls-policies.md), call T036 then T037, return the club/team list plus a short-lived HMAC-signed token (`veoLinkTokenSecret`) carrying the session cookie
- [x] T039 Implement `app/server/api/veo/link.post.ts`: require a session (checked first, before body/token parsing), verify+decode the signed token from T038, upsert `veo_team_mappings` and `veo_sync_credentials` via `useUserDb(event, ...)` — RLS (T034) is the actual team-isolation/trainer-role authorization check here
